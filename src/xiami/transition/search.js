KISSY.add(function(S, Node, Transition, Event, header, suspender, Overlay, ScrollView, ScrollbarPlugin) {
    ...
    var searchTPL = '<input type="text" id="J_searchContent" class="search-txt"/>'+
        '<input type="button" id="J_search" value="" class="search-btn"/>'+
        '<div class="album-loading" style="height: 60px;background-image: none;">'+
        '<ul class="album-list J_searchResult"></ul>'+
        '</div>';
    ...
    return {
        ...
        fetchData: function(key){
            ...
            S.IO({
                dataType: 'jsonp',
                url: 'http://test.fem.taobao.net:3000/search/suggest/' + key,
