KISSY.add("xiami/transition/album", function(S, Node, Transition, Event, header, suspender, Overlay, ScrollView, ScrollbarPlugin) {
  var $ = Node.all, el, myName = this.getName(), body = $("#body"), headerEl = header.getHeader(myName), pid = 0;
  ...
  return{init:function(config) {
    ...
    var _this = this;
    pid = config.id;
    S.IO({dataType:"jsonp", url:"http://test.fem.taobao.net:3000/album/" + config.id, success:function(data) {
      var _html = _this.getTemplate(), _list_html = "", list_songs = data["songs"], albumCover = data["img"];
      ...
      el.all(".J_album_play").on(Event.Gesture.tap, function() {
        var target = $(this), song_id = target.attr("data-id"), location = target.attr("data-location"), title = target.parent("li").one(".album-song-title").text(), cover = albumCover;
      ...
      el.all(".J_album_play_first").on(Event.Gesture.tap, function(e) {
        var target = el.one(".J_album_play"), song_id = target.attr("data-id"), location = target.attr("data-location"), title = target.parent("li").one(".album-song-title").text(), cover = albumCover;
        suspender.playOne({id:song_id, location:location, albumCover:cover, title:title})
      });
    ...
    var _html = ['<div class="album-title">', '<div class="album-img"><img src="{{img}}"></div>', '<div class="album-info album-inline">', "<h3>{{author}}</h3>", '<div class="album-desc J_album_desc">{{desc}}</div>', '<div class="album-control">', '<button class="play inline J_album_play_first">&nbsp;</button>', '<button class="list inline J_album_add_list_all">&nbsp;</button>', "</div>", "</div>", '<div class="album-list-count">{{list_count}}\u9996\u6b4c\u66f2</div>', "</div>"].join("");
    return _html
  }, getListTemplate:function() {
    var _html = ["<li>", '<h3 class="album-song-title">{{title}}</h3>', '<div class="album-btn-group">', '<button class="J_album_play play inline" data-id="{{song_id}}" data-location="{{song_location}}">&nbsp;</button>', '<button class="J_album_add_list list inline" data-id="{{song_id}}" data-location="{{song_location}}">&nbsp;</button>', "</div>", "</li>"].join("");
    return _html
  }, getAlbumDescPopup:function(data) {
    if(Album.pop) {
      return false
    }
    var _html = ['<dl class="desc-container">', '<dt class="desc-header">', '<h4 class="title">{{title}}</h4>', '<h4 class="article">{{author}}</h4>', "</dt>", '<dd class="desc"><div class="ks-scrollview-content ks-content">{{desc}}</div></dd>', "</dl>", '<i class="J_close album-close">\u00d7</i>'].join("");
    _html = _html.replace(new RegExp("{{title}}", "gi"), data["title"]);
    _html = _html.replace(new RegExp("{{desc}}", "gi"), data["desc"]);
    _html = _html.replace(new RegExp("{{author}}", "gi"), data["artist"].title);
    var cfg = {width:document.documentElement.clientWidth * 0.98, height:(document.documentElement.clientHeight - $("#suspender").outerHeight() - document.documentElement.clientHeight * 0.05 - 20) * 0.98, closeable:true, align:{points:["tc", "tc"]}, content:_html, elCls:"album-popup"};
    ...
    Album.pop.on("afterRenderUI", function() {
      (function() {
        ...
        var scrollview = (new ScrollView({srcNode:Album.pop.get("el").all(".desc"), plugins:[new ScrollbarPlugin({})]})).render()
      })();
